...
def plot_behavior_terminal(data_io, exp_dir):
    ...
    """
    # load the trial data
    trial_data_file = exp_dir.joinpath(
    ...
    plot_series = [
        c_mean[::scale_factor].to_list(),
        i_mean[::scale_factor].to_list(),
        o_mean[::scale_factor].to_list(),
    ]  # only plot every nth value, depending on terminal size, to avoid that plotting is messed up by breaking on columns
    config = {
        "height": 10,
        "format": "{:8.0f}",
        "colors": [acp.green, acp.red, acp.lightgray],
    }  # correct is green, incorrect is red, omission gray
    ...
    pump_data_file = exp_dir.joinpath(
