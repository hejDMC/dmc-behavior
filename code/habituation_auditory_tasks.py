...
class Habituation(threading.Thread):
    def __init__(self):
        ...
        self.tone_amplitude = self.task_prefs['task_prefs']['tone_amplitude']  # amplitude of tones and tone cloud
        ...
        self.stim_strength = 100  # in habi always 100, but change this along the training!!
        ...
        self.pump_time_after_audio = self.task_prefs['task_prefs']['pump_time_after_audio']  # in seconds
    ...
    def create_tone_cloud(self, tgt_octave, stim_strength):
        ...
        for i, tone in enumerate(tone_sequence):
            tone_cloud[k:k + int(self.fs * self.tone_duration), i] = create_tone(self.fs, tone, self.tone_duration,
                                                                                 self.tone_amplitude)
    ...
    def data_logger(self):
        ...
        with open(self.trial_data_fn, "a") as log:
            log.write("{0},{1},{2},{3},{4},{5},{6}\n".format(time.time(), str(self.trial_num), str(self.trial_start), str(self.trial_id), str(self.tone_played), str(self.reward_time), str(self.curr_iti)))  # todo: 0: time_stamp, 1: trial_num, 2: trial_start, 3: trial_type:, 4: tone_played, 5: tone_duration, 6: reward_given, 7: inter-trial-intervall
    ...
    def pump_logger(self):
        ...
        with open(self.pump_log, "a") as log:
            log.write("{0},{1}\n".format(time.time(), str(self.pump_duration)))  #  todo: 0: time_stamp, 1: pump_duration
    ...
    def habituation(self):
        ...
        with sd.OutputStream(samplerate=self.fs, blocksize=len(self.cloud), channels=2, dtype='int16',
            ...
            while True:
                if time.time() > self.timeout:  # omission trials: no response in response window
...
habi_day, time_limit = habi_time_limit()  # time_limit in min after which the script automatically stops depending on day of habi
...
while True:
    ...
    if command == "stop":
        ...
        store_meta_data(animal_id, droid, start_time, end_time, exp_dir, task, False, False,
                        procedure="habituation_auditory_tasks", habi_day=habi_day, experimenter=experimenter)
