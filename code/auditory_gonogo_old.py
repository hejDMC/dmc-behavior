...
class AuditoryGoNoGo(threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
    ...
    def stage_checker(self):
        ...
        if self.stage == 0:
            ...
            if (
                self.trial_stat[0] > 150
            ):  # if more than 150 correct trials, check if there were also > 150 correct trials in the previous session
    ...
    def get_trial(self):
        ...
        if self.trial_num > 2:
            if (
                self.tone_history[-3] == self.tone_history[-2] == self.tone_history[-1]
            ):  # limit trial repeats to max 3
    ...
    def get_target_cloud(self):
        ...
        if self.trial_id == "high":
            ...
            self.tgt_octave = 0
        self.cloud = self.create_tone_cloud(self.tgt_octave, curr_stim_strength)
    ...
    def auditory_gonogo(self):
        ...
        if self.choice == "correct":
            ...
            if (
                self.target_position == "no_response"
            ):  # in nogo trials --> play punishment sound
                self.play_tone(
