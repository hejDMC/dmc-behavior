...
from utils.utils import (
    check_dir,
    check_first_day,
    check_ready_for_experiment,
    check_stage_4_advance,
    create_tone,
    get_bias_correction,
    get_stage,
    get_today,
...
class TriggerPulse(threading.Thread):
    def __init__(self):
        ...
        self.trigger_rate = self.droid_settings["base_params"][
            "camera_trigger_rate"
        ]  # in Hz
...
class RotaryRecorder(threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
        self.droid_settings = load_droid_setting()
        self.rotary_rate = self.droid_settings["base_params"][
            "rotary_rate"
        ]  # Hz for sampling
        ...
        self.encoder_left = self.droid_settings["pin_map"]["IN"][
            "encoder_left_rec"
        ]  # pin of left encoder (green wire)
        self.encoder_right = self.droid_settings["pin_map"]["IN"][
            "encoder_right_rec"
        ]  # pin of right encoder (gray wire)
    ...
    def write_sync_data(self):
        ...
        with open(self.fn, "a") as log:
            log.write(
                "{0},{1}\n".format(time.time(), str(self.wheel_position))
            )  #  todo: 0: time_stamp, 1: 2p sync_pulse
...
class SyncRecorder(threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
        self.droid_settings = load_droid_setting()
        self.sync_rate = self.droid_settings["base_params"][
            "2p_sync_rate"
        ]  # Hz for sampling, needs to be at least twice as high as 2p sync freq. (Nyquist)
        self.animal_dir = animal_dir
    ...
    def write_sync_data(self):
        with open(self.fn, "a") as log:
            log.write(
                "{0},{1}\n".format(time.time(), str(self.sync_pulse.get_value()))
            )  #  todo: 0: time_stamp, 1: 2p sync_pulse
...
class Auditory2AFC(threading.Thread):
    def __init__(self):
        ...
        self.stage = get_stage(
            animal_dir, procedure, self.first_day
        )  # current stage of the animal
        self.stage_advance = (
            False  # set this boolean to true if animal can advance a stage
        )
        if self.stage <= 1:
        ...
        self.reward_min_max = self.task_prefs["task_prefs"][
            "reward_size"
        ]  # [max, min] reward size in ul
        self.pump_time = load_pump_calibration()
        self.pump_min_max = [
            p * self.pump_time for p in self.reward_min_max
        ]  # [max, min] pump duration
        ...
        self.encoder_data = Encoder(self.encoder_left, self.encoder_right)
        self.encoder_to_degree = (
            1024 / 360
        )  # the encoder has 1024 steps corresponding to 360 degrees
        self.turning_goal = (
    ...
    def stage_checker(self):
        ...
        if self.stage == 0:
            ...
            self.stage == 1
        ):  # to advance 80 % correct on left and right side respectively
            right_trials = [
                c for d, c in zip(self.decision_history, self.correct_hist) if d == 1
            ]  # get right side trials
            left_trials = [
                c for d, c in zip(self.decision_history, self.correct_hist) if d == -1
            ]  # get right side trials
            ...
            if (
                self.trial_num > 350
            ):  # this criteria feels weird, but in IBL they use 200 trials irrespective of performnce
                self.stage_advance = True
                print(">>>>>  FINISHED STAGE " + str(self.stage) + " !!! <<<<<<<<<")
        elif self.stage == 4:
    ...
    def get_pump_duration(self):
        ...
        if not self.first_day:
            if self.stage == 0:
                ...
                if (
                    pump_data.empty
                ):  # if no previous records of pump duration exist, go for max
                    print(
                        "no previous records of pump duration found, defaulting to max"
                    )
                    pump_duration = self.pump_min_max[0]  # in ms
                else:
                    if (
                        pump_data["pump_duration"].max()
                        == pump_data["pump_duration"].min()
                    ):  # if not bias correction, all trials with same amount of reward, check if reward size is reduced
                        amount_reward = (
                            pump_data["pump_duration"].sum() / self.pump_time
                        )
                        prev_pump_duration = pump_data["pump_duration"].min()
                        if (
                            amount_reward >= 1000
                        ):  # if more than 1 ml reward given reduce pump duration by 0.1 ul = 5 ms;  in IBL task also criteria of min 200 trials, but here animal always needs to do more than 200 trials to get 1000 ul, likewise it will always perform < 200 trials if it received < 1 ml
                        ...
                        ):  # max pump duration is 150 ms = 3 ul
                            pump_duration = self.pump_min_max[0]
                        elif (
                            pump_duration < self.pump_min_max[1]
                        ):  # min pump duration is 75 ms = 1.5 ul
                            pump_duration = self.pump_min_max[1]
                    else:
                        pump_duration = pump_data[
                            "pump_duration"
                        ].min()  # otherwise, keep reward volume constant
        else:
            pump_duration = self.pump_min_max[0]
    ...
    def trigger_reward(self):
        ...
        if (
            ...
            curr_pump_duration = int(
                self.pump_duration
            )  # the usual case, pump_duration is the default one
    ...
    def get_target_cloud(self):
        ...
        if self.trial_id == "high":
            ...
            self.cloud = self.create_tone_cloud(
                self.tgt_octave, self.curr_stim_strength
            )
    ...
    def get_trial_id(self):
        ...
        if self.stage == 0:
            ...
            if (
                self.choice == "incorrect" and self.trial_num > 10
            ):  # if previous trial was incorrect, do the debiasing
    ...
    def choice_evaluation(self):  # , trial_id):
        ...
        )  # self.wheel_position, self.turning_goal) # get the decision from continous stream
    ...
    def pump_logger(self, curr_pump_duration):
        ...
        with open(self.pump_log, "a") as log:
            log.write(
                "{0},{1}\n".format(time.time(), str(curr_pump_duration))
    ...
    def debias(self):
        ...
        if debias_val > 0:  # indicating more right choices
            ...
            tone = list(response_matrix.keys())[
                list(response_matrix.values()).index(bias_side)
            ]
            if tone == "high":
                self.trial_id = "low"
            else:
    ...
    def check_disengage(self):
        ...
        roll_median = (
            pd.DataFrame(self.reaction_times).rolling(20).median()
    ...
    def check_trial_end(self):
        if time.time() > time_out:  # max length reach
            ...
            time.time() > time_out_lt and task.trial_num < low_trial_lim
    ...
    def auditory_2afc(self):
        ...
        while True:
            self.animal_quiet, self.cloud = self.check_quiet_window(
                self.cloud
        ...
        self.data_logger()
        print(
            "trial number: ", self.trial_num, " - correct trials: ", self.trial_stat[0]
        )
