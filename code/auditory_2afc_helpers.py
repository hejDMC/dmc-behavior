...
class StageChecker:
    ...
    def _check_advance_stage_1(self):
        ...
        if np.mean(right_trials) >= self.STAGE_1_CRITERIA and np.mean(left_trials) >= self.STAGE_1_CRITERIA:
            self.stage_advance = True
            self._print_stage_complete(self.stage)
        return self.stage_advance
    ...
    def _check_advance_stage_2(self):
        ...
        right_trials, left_trials = self._get_trial_sides()
        if np.mean(right_trials) >= self.STAGE_2_CRITERIA and np.mean(left_trials) >= self.STAGE_2_CRITERIA:
            self.stage_advance = True
            self._print_stage_complete(self.stage)
        return self.stage_advance
    ...
    def _get_trial_sides(self):
        ...
        right_trials = [c for d, c in zip(self.decision_history, self.correct_hist) if d == 1]
        left_trials = [c for d, c in zip(self.decision_history, self.correct_hist) if d == -1]
    ...
    def _check_stage_4_advance(self):
        ...
        for exp in exp_list:
            ...
            if n_trials > self.LATE_STAGE_NTRIALS and prob_right[0] < self.LATE_STAGE_LAPSE[0] \
        ...
        if cnt == self.LATE_STAGE_COUNT and cnt_stage == self.LATE_STAGE_COUNT:
            ...
            pars, L = mle_fit_psycho(
                np.vstack([np.array(self.STIM_LIST), np.array(num_trials), np.array(prob_right)]),  # 'erf_psycho_2gammas',
                P_model=self.P_MODEL,  # weibull # Gauss error function
                nfits=self.N_FITS
            )

            (bias, slope, gamma1, gamma2) = pars
            if abs(bias - self.STAGE_4_BIAS_CENTER) < self.STAGE_4_BIAS_CRITERIUM and slope < self.SLOPE_CRITERIUM \
                    and gamma1 < self.GAMMA_CRITERIUM and gamma2 < self.GAMMA_CRITERIUM:
                self.stage_advance = True
    ...
    def _check_ready_for_experiment(self, animal_dir):
        ...
        for exp in exp_list:
            ...
            if n_trials > self.LATE_STAGE_NTRIALS:
                ...
                trial_data = pd.concat((trial_data, trial_times))
            for block in [-1, 1]:
                prob_right, num_trials = self._get_performance_per_stim(trial_times, block=block)
                if prob_right[0] < self.LATE_STAGE_LAPSE[0] and prob_right[-1] > self.LATE_STAGE_LAPSE[1]:
                    cnt_perf += 0.5
        if cnt_t_num == self.LATE_STAGE_COUNT and cnt_perf == self.LATE_STAGE_COUNT and cnt_stage == self.LATE_STAGE_COUNT:
            print("trial number and easy trial performance good")
            trial_data = trial_data.reset_index(drop=True)
            bias_stats = {
                'bias_low': 0,
            ...
            for block in [-1, 1]:
                prob_right, num_trials = self._get_performance_per_stim(trial_data, block=block)
                pars, L = mle_fit_psycho(
                    np.vstack([np.array(self.STIM_LIST), np.array(num_trials), np.array(prob_right)]),
                    # 'erf_psycho_2gammas',
                    P_model=self.P_MODEL,  # weibull # Gauss error function
                    nfits=self.N_FITS
                )
            ...
            if bias_shift > self.STAGE_5_BIAS_CRITERIUM and (bias_stats['gamma1_low'] and bias_stats['gamma2_low'] and
                                   bias_stats['gamma1_high'] and bias_stats['gamma2_high']) < self.GAMMA_CRITERIUM:
...
class BiasCorrectionHandler:
    ...
    def _calculate_bias(self, trial_data: pd.DataFrame) -> str:
        ...
        prop_left_choices = left_choices / total_choices if total_choices > 0 else self.DEFAULT_BIAS
